...
# read csv file
stock_df = pd.read_csv("Stock_Stream_Tickers_Data.csv")
...
if(selected == 'Stocks Performance Comparison'):  # if user selects 'Stocks Performance Comparison'
    ...
    dict_csv = pd.read_csv('Stock_Stream_Tickers_Data.csv', header=None, index_col=0).to_dict()[1]  # read csv file
...
# Stock Price Prediction Section Starts Here
    ...
    st.markdown('<p class="big-font">StockStream is a web application that allows users to visualize Stock Performance Comparison, Real-Time Stock Prices and Stock Price Prediction. This application is developed using Streamlit. Streamlit is an open source app framework in Python language. It helps users to create web apps for Data Science and Machine Learning in a short time. This Project is developed by Dipali Gupta.<br>Cheers!</p>', unsafe_allow_html=True)
